...
export default function BlogPostPage() {
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => { /* ... keeps existing logic but maybe we move it to CommentSection? 
     Actually the CommentSection component creates its own isolation. 
     Let's just use the CommentSection component for everything related to comments.
  */ };
  ...
  const fetchComments = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // data is already sorted by API usually, but ensure simple list
        setComments(data);
      }
    } catch (err) {
      console.error(err);
